...
class Dense3DSpatialTransformer(Layer):
	...
	def build(self, input_shape):
		...
		if (input_shape[0][1] != input_shape[1][1] or 
		    input_shape[0][2] != input_shape[1][2] or
		    input_shape[0][3] != input_shape[1][3]):
			raise Exception('Arguments should have same height, width and depth'
	...
	def _transform(self,I,dx,dy,dz):
		...
		#if(self.interp_type == 'trilinear'):
		#	return self._interpolate(I,x_new,y_new,z_new)
		#else:
		#	return self._nn(I,x_new,y_new,z_new)
		return self._interpolate(I,x_new,y_new,z_new)
	...
	def _interpolate(self,im,x,y,z):
		...
		output = tf.reshape(output, tf.stack([-1,out_height,out_width,out_depth,channels]))
		return output

	'''
	def _nn(self,im,x,y,z):
		...
		dim2 = depth*width
		dim1 = depth*width*height
		base = self._repeat(tf.range(num_batch)*dim1, (height-2)*(width-2)*(depth-2))

		idx_a = base + y0*dim2 + x0*dim3 + z0
		...
		output = tf.reshape(Ia, tf.stack([-1,height-2,width-2,depth-2,channels]))
		return output
	'''	
